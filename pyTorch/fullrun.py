...
if __name__ == "__main__":
    ...
    task = Task.init(project_name='MSci-Project', task_name='Regular paths and features')
    ...
    if train_models:
        ...
        for ind, model in enumerate(model2):
            torch.save(model.model, 'IRL_model_'+str(ind)+'.pth')
        tensorboard_writer.close()
    
    irl_models = [torch.load('reg_IRL_model_'+str(ind)+'.pth') for ind in [0,1]] # Load models
