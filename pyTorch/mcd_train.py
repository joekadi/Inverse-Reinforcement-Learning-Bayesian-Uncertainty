...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        print('\n... got dropout value from cmd line ...\n')
         ...
         raise Exception("Dropout p value to train with not supplied")

    # Get number of paths
    if len(sys.argv) > 2:
        num_paths = int(str(sys.argv[2]))
        print('\n... got number of paths from cmd line ...\n')
    else:
         raise Exception("Number of paths to train on not supplied")
    ...
    configuration_dict = {'number_of_epochs': 4, 'base_lr': 0.1, 'p': dropout_val, 'no_hidden_layers': 3, 'no_neurons_in_hidden_layers': len(feature_data['splittable'][0])*2 } #set config params for clearml
    # Configuration_dict = task.connect(configuration_dict) #For when task getting logged on clearML
    ...
    TRAINED_MODELS_PATH = "./regular/models/dropout/"
    ...
    torch.save(model.model, TRAINED_MODELS_PATH+str(worldtype)+'_'+str(dropout_val)+'_'+str(len(example_samples))+'_REG_model.pth') 
