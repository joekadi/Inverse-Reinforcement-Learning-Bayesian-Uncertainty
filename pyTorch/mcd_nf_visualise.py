...
"""
#Print epdue for every pred
for path in paths:
    for dropout_val in dropout_vals:
        for variant in variant_vals:
            epd = torch.mean(torch.abs(results['ow'][path][dropout_val][variant]['y_mc_relu_P']-mdp_solution['p']), 1,True)
            uncertainty =  torch.tensor(results['ow'][path][dropout_val][variant]['y_mc_std_relu'].reshape(len(results['ow'][1024][1.0][0]['y_mc_std_relu']),1)) 
            scaler = MinMaxScaler()
            epd = scaler.fit_transform(epd)
            uncertainty = scaler.fit_transform(uncertainty)
            epd_uncertainty_err = np.mean(np.abs(epd-uncertainty))            
            print('Path :', path, 'P val: ', dropout_val, 'Variant: ', variant, 'epd_uncertainty_err: ', epd_uncertainty_err)
"""

#Calculate and store avg uncertainties and policies for ensemble dropout 
...
ensembles_avg_uncertainty = scaler.fit_transform(ensembles_avg_uncertainty)
  


#Plot ensemble EPDUE
fig, ax = plot_epdue(ensembles_avg_uncertainty, ensemble_avg_epd)
...
dropout_avg_uncertainty = scaler.fit_transform(dropout_avg_uncertainty)

#Plot dropout EPDUE
fig, ax = plot_epdue(dropout_avg_uncertainty, dropout_avg_epd)
...
for path in paths:
    for dropout_val in dropout_vals:
        for variant in variant_vals:
            ...
            this_ence = ence(mdp_solution['p'].squeeze(), policy.squeeze(), uncertainty.squeeze())
           
            print('Path :', path, 'P val: ', dropout_val, 'Variant: ', variant, 'ence: ', this_ence*100)

"""

dropout_ences_per_path = []
dropout_uncertainties_per_paths = []
dropout_policies_per_paths = []

ensembles_ences_per_path = []
ensembles_uncertainties_per_paths = []
ensembles_policies_per_paths = []
...
for path in paths:
    dropout_agg_policy = torch.zeros(256,5)
    dropout_agg_uncertainty = torch.zeros(256)
    dropout_ence = 0.0

    ensemble_agg_policy = torch.zeros(256,5)
    ensemble_agg_uncertainty = torch.zeros(256)
    ensemble_ence = 0.0
    ...
    for variant in variant_vals:
        ...
        scaler = MinMaxScaler()
        dropout_policy = scaler.fit_transform(dropout_policy)
        dropout_uncertainty = scaler.fit_transform(dropout_uncertainty.reshape(len(dropout_uncertainty), 1))

        ensemble_policy = scaler.fit_transform(ensemble_policy)
        ...
        dropout_ence += (ence(mdp_solution['p'].squeeze(), dropout_policy.squeeze(), dropout_uncertainty.squeeze()))
        ensemble_ence += (ence(mdp_solution['p'].squeeze(), ensemble_policy.squeeze(), ensemble_uncertainty.squeeze()))


    print('Dropout', 'Path :', path, 'Variant: ', variant, 'ence: ', dropout_ence/4)
    print('Ensemble', 'Path :', path, 'Variant: ', variant, 'ence: ', ensemble_ence/4)

    dropout_ences_per_path.append(dropout_ence/4)
    dropout_uncertainties_per_paths.append(dropout_agg_uncertainty/4)
    dropout_policies_per_paths.append(dropout_agg_policy/4)

    ensembles_ences_per_path.append(ensemble_ence/4)
    ensembles_uncertainties_per_paths.append(ensemble_agg_uncertainty/4)
    ensembles_policies_per_paths.append(ensemble_agg_policy/4)


dropout_avg_ence = sum(dropout_ences_per_path)/len(dropout_ences_per_path)
ensemble_avg_ence = sum(ensembles_ences_per_path)/len(ensembles_ences_per_path)
print('Dropout avg ence = ', dropout_avg_ence)
print('Ensemble avg ence = ', ensemble_avg_ence)

dropout_avg_policy = (sum(dropout_policies_per_paths)/len(dropout_policies_per_paths)).squeeze()
dropout_avg_uncertainty = (sum(dropout_uncertainties_per_paths)/len(dropout_uncertainties_per_paths)).squeeze().detach().numpy()

ensemble_avg_policy = (sum(ensembles_policies_per_paths)/len(ensembles_policies_per_paths)).squeeze()
ensemble_avg_uncertainty = (sum(ensembles_uncertainties_per_paths)/len(ensembles_uncertainties_per_paths)).squeeze().detach().numpy()



#Calculate ence from avg policies and avg uncertainties
ensemble_ence = ence(mdp_solution['p'].squeeze(), ensemble_avg_policy, ensemble_avg_uncertainty)*1000
print('ensemble ence = ', ensemble_ence)

#Plot ence
fig, ax = plot_ence(mdp_solution['p'].squeeze(), ensemble_avg_policy, ensemble_avg_uncertainty)
textstr = 'ENCE = ' + str(round(ensemble_ence,3))
props = dict(boxstyle='round', facecolor='white', alpha=0.75)
ax.text(0.96, 0.1,textstr, transform=ax.transAxes, fontsize=9,
        verticalalignment='top',
        horizontalalignment='right',
        bbox=props
        )
...
#Calculate ence from avg policies and avg uncertainties
dropout_ence = ence(mdp_solution['p'].squeeze(), dropout_avg_policy, dropout_avg_uncertainty)*1000
print('dropout ence = ', dropout_ence)


#Plot ence
fig, ax = plot_ence(mdp_solution['p'].squeeze(), dropout_avg_policy, dropout_avg_uncertainty)
textstr = 'ENCE = ' + str(round(dropout_ence,3))
props = dict(boxstyle='round', facecolor='white', alpha=0.75)
ax.text(0.96, 0.1,textstr, transform=ax.transAxes, fontsize=9,
        verticalalignment='top',
        horizontalalignment='right',
...
'''
#tester
test_std = np.zeros((256))
test_p = mdp_solution['p'].squeeze()
test_std = [x + 0.01 for x in test_std]
test_p += 0.01
test_ence = ence(mdp_solution['p'].squeeze(), test_p, test_std)
print('test ence = ', test_ence)
#Test
fig, ax = plot_ence(mdp_solution['p'].squeeze(), test_p, test_std)
textstr = 'ENCE = ' + str(round(test_ence,4))
props = dict(boxstyle='round', facecolor='white', alpha=0.75)
ax.text(0.96, 0.1,textstr, transform=ax.transAxes, fontsize=9,
        verticalalignment='top',
        horizontalalignment='right',
...
ax.legend(loc='center right', prop={'size': 6})    
fig.tight_layout()
fig.savefig(GRAPHS_PATH + "test_ence.png")
'''
...
#Plot Uncertainty (Y) vs Paths (X) line graph figure for each dropout value (colour) as a function of paths
#To show that it is dropout causing uncertainty calibration. Expected to see lower uncertainty for dropout 0.0 and highest for 1.0
...
ax1.plot(paths, uncertainty_vals[1], alpha=0.8, label='0.5 MC Dropout')
ax1.plot(paths, uncertainty_vals[2], alpha=0.8, label='1.0 MC Dropout')
ax1.plot(paths, uncertainty_vals[3], alpha=0.8, label='SWAG')
