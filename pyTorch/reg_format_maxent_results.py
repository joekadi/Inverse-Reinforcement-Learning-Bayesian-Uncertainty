...
filename = 'ow_'+str(num_paths)+'_results.pkl'
open_file = open('/Users/joekadi/Documents/University/5thYear/Thesis/Code/MSci-Project/pyTorch/regular/results/maxent/'+filename, "rb")


open_results = pickle.load(open_file)
open_file.close()
y_mc_relu_reward = open_results[0]
y_mc_relu = y_mc_relu_reward[:,0]
EVD = open_results[1]

y_mc_relu_reward = y_mc_relu_reward

y_mc_relu_v, y_mc_relu_q, y_mc_relu_logp, y_mc_relu_P = linearvalueiteration(mdp_data, y_mc_relu_reward)

NP_results = [y_mc_relu, None, y_mc_relu_reward, y_mc_relu_v, y_mc_relu_P, y_mc_relu_q, EVD, 10, 5,000]
