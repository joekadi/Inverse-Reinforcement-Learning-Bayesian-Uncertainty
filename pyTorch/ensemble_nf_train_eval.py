...
def run_NN_ensemble(models_to_train, X, configuration_dict):
    ...
    # Create paths
    TRAINING_PATH = "./noisey_features/models/ensembles/"
    ...
    for model_name in model_names:
        ...
        # compute evd & loss
        test_loss = NLL.apply(y_hat_test, initD, mu_sa, muE, x_test, mdp_data)
        test_evd = NLL.calculate_EVD(truep, torch.matmul(x_test, y_hat_test))
    ...
    ensemble_loss, model_weights = ensemble_selector(
        loss_function=NLL, optim_for_loss=True, y_hats=y_hats_test, X=x_test, init_size=1, replacement=True, max_iter=10)
    ...
    for _, row in model_weights.iterrows():
        ...
        ensemble_loss_test.append(NLL.apply(tmp_y_hat, initD, mu_sa, muE, x_test, mdp_data).detach().numpy())
    ensemble_loss_test = pd.Series(ensemble_loss_test)

    # Compute loss of an ensemble which equally weights each model in the pool
    losses = []
    for model, predictedR in y_hats_test.items():losses.append(NLL.apply(predictedR, initD, mu_sa, muE, x_test, mdp_data).detach().numpy())
    ...
    ensemble_acc, model_weights = ensemble_selector(
        loss_function=NLL, optim_for_loss=False, y_hats=y_hats_test, X=x_test, init_size=1, replacement=True, max_iter=10)

    # Compute evd of the equally weighted ensemble
    evds = []
    for model, predictedR in y_hats_test.items():
        evds.append(NLL.calculate_EVD(truep, torch.matmul(x_test, predictedR)).detach().numpy())
...
if __name__ == "__main__":
    ...
    NF_results = [average_predictions.detach().numpy(), predictions_uncertainty.detach().numpy()]

    #Save results
    print('\n... saving results ...\n')

    # Create path for trained models
    RESULTS_PATH = "./noisey_features/results/ensembles/"
