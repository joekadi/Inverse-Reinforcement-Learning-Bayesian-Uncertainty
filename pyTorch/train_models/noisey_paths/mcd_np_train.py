...
def save_vars_for_GP(mdp_data, example_samples, feature_data, mdp_solution):
    """
    ...
    GP_VARS_PATH = "./noisey_paths/params_for_GP/"
    ...
    file_name = GP_VARS_PATH+str(worldtype)+'_'+ str(num_paths)+'_NP_model_'+str(index_states_to_remove)+'.mat' 
...
if __name__ == "__main__":
    ...
    #To save noisey variables in .mat format for matlab GPIRL
    #Kills  process, so by default is commented out

    #save_vars_for_GP(mdp_data, example_samples, feature_data, mdp_solution)
    ...
    TRAINED_MODELS_PATH = "./noisey_paths/models/dropout/"
    ...
    torch.save(model.model, TRAINED_MODELS_PATH+str(worldtype)+'_'+str(dropout_val)+'_'+ str(num_paths)+'_NP_model_'+str(index_states_to_remove)+'.pth') 
